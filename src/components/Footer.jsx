  ...
  return (
    <div className=' w-full font-[Satoshi]'>
      <div className=' max-w-screen-xl mx-auto flex py-10 gap-40'>
        ...
        {["Privacy Policy", "Cookie Policy", "Impressum" , "Terms"].map((elem,index)=><a className=' mr-8 text-zinc-600 pl-2 text-sm font-medium'>{elem}</a>)}
        </div>
        <div className=' basis-1/2 flex gap-4'>
            <div className=' basis-1/3'>
                <h4 className='mb-6 text-zinc-500 capitalize text-sm'>socials</h4>
                {["instagram","twitter (X?)","LinkedIn"].map((item,index)=><a className='block mt-2 capitalize text-zinc-600 text-sm'>{item}</a>)}
            </div>
            <div className=' basis-1/3'>
                <h4 className='mb-6 text-zinc-500 capitalize text-sm'>sitemap</h4>
                {["home","work","careers","contact"].map((item,index)=><a className='block mt-2 capitalize text-zinc-600 text-sm'>{item}</a>)}
            </div>
